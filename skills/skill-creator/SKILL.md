...
5. **Master Suite Integration (Dynamic References)** - Capability to scaffold nested reference chapters inside a master suite's `references/` directory instead of creating a standalone top-level skill, enforcing word counts (<2k words) and enabling lazy loading.
