...
- 任務計畫強制落盤：所有計畫必須實體寫入 `01_Work/task.md`，嚴禁僅在對話框輸出。
- 管線追蹤：`task.md` 的頂部必須包含 `[Pipeline Trace]: Orchestrator > Router > [Sub-Agent Name]`。

## 持久化與 GC (強制)

- 多步任務：在 01_Work/ 建立 session_state.json。
- Pointers Only：僅存路徑指針，嚴禁寫入全量數據。
- GC 觸發標記：Sub-Agent 完成任務時，必須在 `task_results.md` 結尾附上 `[Action: Trigger GC]` 標籤。
- 任務完成 (GC)：Orchestrator 偵測到 GC 標籤後，必須主動清空 `session_state.json`，並將成果移至 `02_Output`。
...
## Agent 與技能管理 (Agent & Skill Management - 強制)

- **Single Source of Truth**: 所有 Agent 與業務邏輯統一以「技能 (Skill)」形式存放於 `~/.gemini/antigravity/skills/` 目錄。
- **格式規範**: 必須遵循 `skills/[kebab-case-name]/SKILL.md` 結構。
- **Legacy Path**: `~/.gemini/antigravity/workflows/` 目錄僅供舊版相容與備份參考，嚴禁作為新功能開發的定義來源。
- 維持 `skills_registry.json` 為唯一路由索引。
